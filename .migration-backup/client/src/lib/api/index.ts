...
export const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || 'https://crypto-signal-bot-deqd.onrender.com';
...
export const api = {
  ...
  getMetrics: async (): Promise<Metric[]> => {
    try {
      ...
      console.warn("Using mock metrics fallback", e);
  ...
  getRecentSignals: async (): Promise<Signal[]> => {
    try {
      ...
      console.warn("Using mock signals fallback", e);
      return new Promise((resolve) => setTimeout(() => resolve([
  ...
  getSystemAlerts: async (): Promise<SystemAlert[]> => {
    try {
      ...
      console.warn("Using mock alerts fallback", e);
      return new Promise((resolve) => setTimeout(() => resolve([
  ...
  getWatchlist: async () => {
    try {
      ...
      console.warn("Using mock watchlist fallback", e);
      return new Promise((resolve) => setTimeout(() => resolve([
  ...
  getActivePositions: async (): Promise<Position[]> => {
    try {
      ...
      console.warn("Using mock active positions fallback", e);
      return new Promise((resolve) => setTimeout(() => resolve([
  ...
  getClosedPositions: async (): Promise<Position[]> => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/positions/closed`);
      if (!response.ok) throw new Error('Failed to fetch closed positions');
      return response.json();
    } catch (e) {
      console.warn("Using mock closed positions fallback", e);
      return new Promise((resolve) => setTimeout(() => resolve([
  ...
  getHealth: async (): Promise<{ modules: ModuleHealth[], logs: SystemLog[], uptime: string }> => {
    try {
      ...
      console.warn("Using mock health fallback", e);
      return new Promise((resolve) => setTimeout(() => resolve({
        uptime: "14d 02h 45m",
        modules: [
